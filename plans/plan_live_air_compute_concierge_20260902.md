...
- Implementation compatibility testing selected `qwen3-coder-30b-a3b-instruct` for all three configurable roles. One complete live run measured 6,951 ms for intake, 2,928 ms for critique, and 8,864 ms for diagnosis; these are point measurements, not guarantees.
...
|---|---|---|---|
| Planner | `qwen3-coder-30b-a3b-instruct` | `qwen3-coder-next`, `north-mini-code` | Valid JSON rate, valid job-spec rate, median latency <=20 seconds |
...
The user approved a compact newcomer experience after reviewing the recommendation-card UI. This addendum adds: evidence-bound AIR typo suggestions; exact-line AIR script explanations with deterministic fallback; resource arithmetic; first-run measurement guidance; deterministic environment checks; beginner mistake warnings; and job inspect/cancel lifecycle commands. These features must use compact recommendation rows and Script, Explain, Check, and Run tabs so only one output task consumes vertical space at a time. Existing confirmation, redaction, validation, and no-execution boundaries remain unchanged.
